...
error_reporting(0);
// database ke table
$data=mysql_query("select * from jadwal_dokter order by kodedokter");
...
<div>
  ...
  <tr>
    ...
    <td width="40%"><a href="?page=data_jadwal_dokter" title="refresh">[Refresh]</a>&nbsp;&nbsp;<a href="?page=input_jadwal_dokter">[+Tambah]</a>&nbsp;&nbsp;<a href="simpanjadwaldokterxls.php">[Save to.excel]</a></td>
    <td width="9%">&nbsp;</td>
  </tr>
</table>
</form>
...
<form method="post" action="?page=deleteall_jadwal_dokter"> 
<?php
...
$jum_data = mysql_num_rows(mysql_query("SELECT * FROM jadwal_dokter"));
...
if ($halaman>1)
{
	echo "<a href='index.php?page=data_jadwal_dokter&hal=$back'>Back</a>";
}
...
if ($halaman<$jum_page)
{
	echo "<a href='index.php?page=data_jadwal_dokter&hal=$next'>Next</a>";
}
      ...
      <td height="64"><table width="100%" height="74" cellpadding="2" cellspacing="1">
       ...
       <td width="4%" height="42">&nbsp;</td>
         <td width="4%">NO</td>
          ...
          <td width="25%">NAMA POLIKELINIK</td>
...
{
	$search = $_POST['search'];
	$sql1 = mysql_query("SELECT * FROM jadwal_dokter WHERE kodedokter LIKE '%$search%'")or die(mysql_error());
	}
	else{
	$sql1 = mysql_query("SELECT * FROM jadwal_dokter  order by kodedokter asc LIMIT $mulai,$jum_hal");
	}
      ...
      <tr bgcolor="#CCCCCC"><td height="27" align="center"><input type="checkbox" name="check_list[]" value="<?php echo $tampil['kodedokter'];?>"/></td>
       <td><div align="center"><?php echo $no=$no+1;?></td>
        ...
        <td><a href="?page=detail_dokter&kodedokter=<?php echo $tampil['kodedokter'];?>"><?php echo $tampil['nmdokter']?></a></td>
		     <td align="center"><?php echo $tampil['namapoll'];?></td>
          <td width="6%"><div align="center"><a href="?page=edit_jadwal_dokter&kodedokter=<?php echo $tampil['kodedokter'];?>">Edit</a></div></td>
          <td width="6%"><div align="center"><a href="?page=delete_jadwal_dokter&kodedokter=<?php echo $tampil['kodedokter'];?>" onClick="return confirm('Apakah anda yakin ingin menghapus data ini ?')">Hapus</a></div></td>
          </tr>
        <?php
  }
}
