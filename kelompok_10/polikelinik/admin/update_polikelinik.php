...
else
{
	if (!empty($_FILES["photo"]["tmp_name"]))
	{
		...
		{           
			...
			mysql_query("UPDATE polikelinik WHERE kodepoll='$kodepoll' LIMIT 1");
		} 
		else { ?><script language="javascript">alert("Jenis gambar yang anda kirim salah. Harus .jpg .gif .png, image sementara akan di kosongkan");location.href="index.php?page=data_polikelinik";</script><?php }
	}
	$masukan=mysql_query("UPDATE polikelinik SET kodepoll='$kodepoll',namapoll='$namapoll' WHERE kodepoll='$kodepoll'");
	if($masukan){
		?><script language="javascript">alert("Data berhasil di ubah !"); location.href="?page=data_polikelinik";</script><?php
		}else
		{
