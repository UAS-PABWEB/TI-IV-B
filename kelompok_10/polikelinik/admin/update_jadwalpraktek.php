...
else
{
	if (!empty($_FILES["photo"]["tmp_name"]))
	{
			...
			else { ?><script language="javascript">alert("Jenis gambar yang anda kirim salah. Jarus .jpg . gif .png, image sementara akan di kosongkan"):location.href="index.php?page=datajadwalpraktek";</script><?php }
			}
			$masukan=mysql_query("UPDATE jadwalpraktek SET kodejadwal='$kodejadwal',hari='$hari',jammulai='$jammulai',jamselesai='$jamselesai',kodedokter='$kodedokter[kodedokter]' WHERE kodejadwal='$kodejadwal'");
			if($masukan){
				?><script language="javascript">alert("Data Berhasil di ubah !"); location.href="?page=data_jadwalpraktek";</script><?php
				}else
				{
				?><script language="javascript">alert("Gagal diubah !"); location.href="javascript:history.back()";</script><?php
				}
